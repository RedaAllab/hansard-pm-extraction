...
| Dashboard visualization pass (remaining) | Not started | Stylometric radar (unstable min-max normalization, no sample-size cue for Truss), plus new charts (TF-IDF distinctive terms, PMQs-vs-other splits, before/after crisis box plots) — full critique and priority ranking discussed, not yet implemented. |
