...
| Dashboard visualization pass | Done, in `hansard-pm-nlp` (`app/app.py`, `src/hansard_pm_nlp/dashboard_helpers.py`) | Radar fix (normalizes to the *selected* PMs, not the full profile, so deselecting Liz Truss actually rescales the rest; legend now shows each PM's n). Five new sections: TF-IDF distinctive terms per PM, confusion-matrix click-to-drill-down, crisis-vs-baseline box plots for H2/H3 (reusing the exact regression dummies), a topic x PM heatmap, and a PMQs-vs-other-debates split (finally wires up a `load_affect()` loader that existed since v1 but was never called). All transform logic extracted to a tested module rather than inlined. MTLD-over-time is the one item from the original critique not done this session — it needs new binned computation (not just visualization of an existing column), flagged for next session. |
